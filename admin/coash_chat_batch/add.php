...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($formerror)) {
        $stmt = $connect->prepare("INSERT INTO chat (from_person,to_person, msg,msg_files,date,send_type,coash_id)
            VALUES(:zfrom_person,:zto_person,:zmessage,:zmsg_files,:zdate,:zsend_type,:zcoash_id)");
        $stmt->execute(array(
        ...
        if ($stmt) {
            header('LOCATION:main.php?dir=coash_chat_batch&page=chat&ind_username=' . $to_person); ?>
            ?>
<?php
        }
    }
